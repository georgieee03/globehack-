...
@MainActor
protocol InsforgeServiceProtocol {
...
enum InsforgeServiceError: LocalizedError {
    case missingUser
    case missingProfile
    case missingSession
...
enum HydraRuntime {
    ...
    static let shouldUseLiveServices: Bool = {
        #if canImport(InsForge)
        !isPreview && HydraInsforgeCore.shared != nil
        #else
        false
        #endif
    }()
...
@MainActor
final class MockInsforgeService: InsforgeServiceProtocol {
    static let shared = MockInsforgeService()

    private let liveService: LiveInsforgeService?
    ...
    init(useLiveServices: Bool? = nil) {
        if useLiveServices ?? HydraRuntime.shouldUseLiveServices {
            liveService = try? LiveInsforgeService()
        } else {
            liveService = nil
        }
    }
    ...
    func fetchClientProfile(userID: UUID) async throws -> ClientProfile {
        ...
        guard let profile = profilesByUserID[userID] else {
            throw InsforgeServiceError.missingProfile
        }
...
@MainActor
final class HydraInsforgeCore {
    static let shared: HydraInsforgeCore? = try? HydraInsforgeCore()

    let client: InsForgeClient

    init() throws {
        guard
            let url = HydraScanConstants.insforgeURL,
            !HydraScanConstants.insforgeAnonKey.isEmpty
        else {
            throw InsforgeServiceError.unavailable("Set INSFORGE_URL and INSFORGE_ANON_KEY before using the live service.")
        }

        client = InsForgeClient(
            baseURL: url,
            anonKey: HydraScanConstants.insforgeAnonKey,
    ...
    func requireCurrentSessionContext() async throws -> HydraSessionContext {
        ...
        throw InsforgeServiceError.missingUser
    }
    ...
    func loadSessionContext(from authUser: User, allowProvisioningRetry: Bool) async throws -> HydraSessionContext {
        ...
        throw InsforgeServiceError.incompleteOnboarding
    }
...
@MainActor
final class LiveInsforgeService: InsforgeServiceProtocol {
    ...
    func updateClientProfile(_ profile: ClientProfile) async throws -> ClientProfile {
        ...
        if let context = await HydraSessionStore.shared.currentContext(), context.userID == saved.userID {
            await HydraSessionStore.shared.update(date(
                HydraSessionContext(
    ...
    func createAssessment(_ assessment: Assessment) async throws -> Assessment {
        ...
        guard var saved = rows.first else {
            throw InsforgeServiceError.unavailable("The assessment was submitted but no row was returned.")
        }
    ...
    func createOutcome(_ outcome: Outcome) async throws -> Outcome {
        ...
        guard let resolvedSession = try await resolveSession(for: outcome.sessionID, clientProfileID: clientProfileID) else {
            throw InsforgeServiceError.missingSession
        }
        ...
        guard response.success else {
            throw InsforgeServiceError.unavailable("Outcome submission did not complete successfully.")
        }
        ...
        guard var saved = rows.first else {
            throw InsforgeServiceError.unavailable("The outcome was recorded but could not be read back.")
        }
    ...
    func createCheckin(_ checkin: DailyCheckin) async throws -> DailyCheckin {
        ...
        guard response.success else {
            throw InsforgeServiceError.unavailable("The check-in did not complete successfully.")
        }
...
final class LiveInsforgeService: InsforgeServiceProtocol {
    init() throws {
        throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.")
    }

    func currentSessionContext() async -> HydraSessionContext? { nil }
    func ensureClientProfile(for user: HydraUser) async throws -> ClientProfile { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchClientProfile(userID: UUID) async throws -> ClientProfile { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func updateClientProfile(_ profile: ClientProfile) async throws -> ClientProfile { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func createAssessment(_ assessment: Assessment) async throws -> Assessment { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchAssessments(clientID: UUID) async throws -> [Assessment] { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchLatestAssessment(clientID: UUID) async throws -> Assessment? { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchSessions(clientID: UUID, limit: Int) async throws -> [HydraSession] { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchLatestSession(clientID: UUID, statuses: [HydraSessionStatus]?) async throws -> HydraSession? { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchSessionAwareness(clientID: UUID) async throws -> HydraSessionAwareness { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func sessionAwarenessStream(clientID: UUID) -> AsyncThrowingStream<HydraSessionAwareness, Error> {
        AsyncThrowingStream { continuation in
            continuation.finish(throwing: InsforgeServiceError.unavailable("InsForge is unavailable in this build."))
        }
    }
    func createOutcome(_ outcome: Outcome) async throws -> Outcome { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchLatestOutcome(clientID: UUID) async throws -> Outcome? { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func createCheckin(_ checkin: DailyCheckin) async throws -> DailyCheckin { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchRecentCheckins(clientID: UUID, limit: Int) async throws -> [DailyCheckin] { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchRecoveryScore(clientID: UUID) async throws -> RecoveryScore { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func fetchRecoveryTrend(clientID: UUID) async throws -> [RecoveryScoreTrendPoint] { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func claimClinicInvite(inviteCode: String, fullName: String) async throws -> HydraSessionContext { throw InsforgeServiceError.unavailable("InsForge is unavailable in this build.") }
    func resetSessionContext() async {}
}
#endif
