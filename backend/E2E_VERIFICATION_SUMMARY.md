...
- Auth → Device Lookup → State Validation → Safe Envelope → Payload Build → Simulation → Status Update → Audit Log
...
1. **Authentication Layer** (`backend/supabase/functions/_shared/supabase-client.ts`)
...
2. **Device State Management** (`backend/supabase/functions/_shared/device-state.ts`)
...
3. **Safe Envelope Validation** (`backend/supabase/functions/_shared/safe-envelope.ts`)
...
4. **Hydrawav Auth Edge Function** (`backend/supabase/functions/hydrawav-auth/index.ts`)
   - Hydrawav3 API authentication proxy
   - Clinic-scoped token storage
   - Simulation mode support

5. **Hydrawav MQTT Edge Function** (`backend/supabase/functions/hydrawav-mqtt/index.ts`)
...
**File**: `backend/supabase/functions/hydrawav-mqtt/index.test.ts`

Comprehensive Deno test suite covering:
- Complete lifecycle flow (idle → in_session → paused → in_session → idle)
...
#### Device Lookup → State Validation
...
#### State Validation → Safe Envelope
...
✅ **Verified**: MQTT proxy publishes commands to Hydrawav3 API endpoint (simulated in test mode)

### Requirement 15.1: Simulation Mode Detection
✅ **Verified**: System operates in simulation mode when HYDRAWAV_API_BASE_URL is unset or "simulation"

### Requirement 15.2: Simulation Mode Processing
✅ **Verified**: Simulation mode processes commands identically to live mode but skips HTTP call

### Requirement 15.3: Simulation Mode Response
✅ **Verified**: Simulation mode returns success response with `simulated: true` flag
...
### Positive Flow Tests
1. ✅ Complete lifecycle: idle → start → in_session → pause → paused → resume → in_session → stop → idle
2. ✅ Authentication and JWT validation
3. ✅ Device lookup with clinic isolation
4. ✅ Valid state transitions
5. ✅ Safe envelope validation passes for valid configs
6. ✅ Payload construction (full and minimal)
7. ✅ Simulation mode execution
8. ✅ Device status updates
9. ✅ Audit log entries created

### Negative Flow Tests
1. ✅ Invalid state transitions rejected (HTTP 409)
2. ✅ Safe envelope violations rejected (HTTP 400)
3. ✅ Violations array includes all parameter violations
4. ✅ Region-specific safe envelope enforced
5. ✅ Unauthenticated requests rejected (HTTP 401)
6. ✅ Cross-clinic access prevented (RLS)
...
export SUPABASE_URL="http://127.0.0.1:54321"  # or your Supabase URL
...
```bash
cd backend
deno test --allow-net --allow-env supabase/functions/hydrawav-mqtt/index.test.ts
```
...
- ✅ Authentication layer validates users and enforces role-based access
- ✅ Device lookup respects multi-tenant clinic isolation
- ✅ State machine validates all transitions
- ✅ Safe envelope prevents unsafe parameter values
- ✅ Payload construction handles all command types correctly
- ✅ Simulation mode works identically to live mode
- ✅ Device status updates reflect command execution
- ✅ Audit logging captures complete command history
...
1. `backend/supabase/functions/hydrawav-mqtt/index.test.ts` - Comprehensive integration test suite
