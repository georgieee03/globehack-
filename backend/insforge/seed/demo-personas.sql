...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════════════════════
  -- Create auth users (simplified — in production these come from InsForge Auth)
  -- ═══════════════════════════════════════════════════════════════════════════
  ...
  VALUES
    (
      v_alex_auth_id,
      'alex.rivera@demo.hydrascan.app',
      crypt('demo-password', gen_salt('bf')),
      true,
      now() - interval '60 days',
      now(),
      '{"name":"Alex Rivera"}'::jsonb,
      jsonb_build_object('role', 'client', 'clinic_id', v_clinic_id::text, 'auth_provider', 'email'),
      false,
      false
    ),
    (
      v_jordan_auth_id,
      'jordan.chen@demo.hydrascan.app',
      crypt('demo-password', gen_salt('bf')),
      true,
      now() - interval '45 days',
      now(),
      '{"name":"Jordan Chen"}'::jsonb,
      jsonb_build_object('role', 'client', 'clinic_id', v_clinic_id::text, 'auth_provider', 'email'),
      false,
      false
    ),
    (
      v_sam_auth_id,
      'sam.patel@demo.hydrascan.app',
      crypt('demo-password', gen_salt('bf')),
      true,
      now() - interval '30 days',
      now(),
      '{"name":"Sam Patel"}'::jsonb,
      jsonb_build_object('role', 'client', 'clinic_id', v_clinic_id::text, 'auth_provider', 'email'),
      false,
      false
    )
  ...
  INSERT INTO auth.user_providers (id, user_id, provider, provider_account_id, provider_data, created_at, updated_at)
  VALUES
    (gen_random_uuid(), v_alex_auth_id, 'email', 'alex.rivera@demo.hydrascan.app', jsonb_build_object('email', 'alex.rivera@demo.hydrascan.app', 'sub', v_alex_auth_id::text), now(), now()),
    (gen_random_uuid(), v_jordan_auth_id, 'email', 'jordan.chen@demo.hydrascan.app', jsonb_build_object('email', 'jordan.chen@demo.hydrascan.app', 'sub', v_jordan_auth_id::text), now(), now()),
    (gen_random_uuid(), v_sam_auth_id, 'email', 'sam.patel@demo.hydrascan.app', jsonb_build_object('email', 'sam.patel@demo.hydrascan.app', 'sub', v_sam_auth_id::text), now(), now())
