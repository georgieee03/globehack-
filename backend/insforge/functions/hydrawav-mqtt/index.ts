...
async function insertAuditLog(
  ...
  const { error } = await insforge.from("mqtt_command_log").insert({
...
serve(async (request) => {
  ...
  try {
    const context = await requireAuthenticatedUser(request, insforge);
    ...
    const clinicToken = await fetchClinicToken(insforge, context.clinicId);
