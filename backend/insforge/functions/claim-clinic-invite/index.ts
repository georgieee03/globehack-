...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data, error: authError } = await insforge.auth.getUser(token);
    ...
    const { data: existingUser, error: existingUserError } = await insforge
      .from("users")
    ...
    const { data: invite, error: inviteError } = await insforge
      .from("clinic_invites")
    ...
    const { data: claimedInvite, error: claimError } = await insforge
      .from("clinic_invites")
      .update({
    ...
    const profile = isRecord(authUser.profile)
      ? authUser.profile
      : {};
    const metadata = isRecord(authUser.metadata)
      ? authUser.metadata
      : {};
    const authProvider =
      (Array.isArray(authUser.providers) &&
          typeof authUser.providers[0] === "string" &&
          authUser.providers[0]) ||
      (typeof metadata.auth_provider === "string" &&
    ...
    const { error: insertUserError } = await insforge
      .from("users")
      .insert({
        id: authUser.id,
    ...
    if (insertUserError) {
      if (insertUserError.code === "23505") {
        const { data: concurrentUser } = await insforge
          .from("users")
          .select("id, clinic_id, role")
          .eq("id", authUser.id)
          .maybeSingle();
    ...
    const { data: clientProfile, error: clientProfileError } = await insforge
