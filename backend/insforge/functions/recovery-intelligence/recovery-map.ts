...
// ---------------------------------------------------------------------------
// suggestGoal Ã¢â‚¬â€ Suggest a RecoveryGoal based on dominant signal type/severity
// ---------------------------------------------------------------------------

/**
 * Suggest a RecoveryGoal based on the dominant signal type and maximum severity.
 *
 * - stiffness / restriction Ã¢â€ â€™ mobility
 * - soreness with severity Ã¢â€°Â¥ 7 Ã¢â€ â€™ relaxation
 * - tightness Ã¢â€ â€™ warm_up
 * - guarding Ã¢â€ â€™ relaxation
 * - default Ã¢â€ â€™ recovery
 */
...
// ---------------------------------------------------------------------------
// findBilateralPartner Ã¢â‚¬â€ Find the bilateral partner for a given region
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// scoreOutcome Ã¢â‚¬â€ Score a single session outcome for prior session summary
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// generateRecoveryMap Ã¢â‚¬â€ Main entry point
// ---------------------------------------------------------------------------
 ...
 * 3. Flag regions with ROM decrease > 5Ã‚Â° as declining trend
...
export async function generateRecoveryMap(
  ...
  // Fetch assessment
  const { data: assessment } = await insforge
    .from("assessments")
    .select("*")
    .eq("id", assessmentId)
    .maybeSingle();
  ...
  const { data: clientProfile } = await insforge
  ...
  const { data: prevAssessments } = await insforge
    .from("assessments")
  ...
  const { data: recentSessions } = await insforge
    .from("sessions")
    .select(`
