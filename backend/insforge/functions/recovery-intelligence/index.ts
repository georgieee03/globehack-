/**
 * Recovery Intelligence Edge Function Ã¢â‚¬â€ Action Router
 *
...
import { requireAuthenticatedUser, requireRole, HttpError, createInsforgeAdminClient } from "../_shared/insforge-client.ts";
...
async function resolveClientProfileIdForUser(
  insforge: ReturnType<typeof createInsforgeAdminClient>,
  userId: string,
  clinicId: string,
) {
...
// ---------------------------------------------------------------------------
// handleRecommend Ã¢â‚¬â€ Full recommendation pipeline
// ---------------------------------------------------------------------------
...
async function handleRecommend(
  ...
  const { data: assessment, error: assessError } = await insforge
    .from("assessments")
    .select("*")
    .eq("id", assessmentId)
    .eq("clinic_id", clinicId)
  ...
  // History Scorer
  const historyResult = await scoreHistory(clientId, insforge);
  ...
  const { sessionConfig, clampingLog } = buildConfig({
    ...
    mac: "", // Placeholder Ã¢â‚¬â€ set during device selection
  });

  // Recovery Map
  const recoveryMap = await generateRecoveryMap(clientId, assessmentId, insforge);

  // Recovery Score (recompute and insert)
  const recoveryScore = await recomputeAndInsertRecoveryScore(insforge, clinicId, clientId);
...
// ---------------------------------------------------------------------------
// Edge Function entry point Ã¢â‚¬â€ Deno.serve with action-based routing
// ---------------------------------------------------------------------------
...
Deno.serve(async (req: Request) => {
  ...
  const insforge = createInsforgeAdminClient();

  try {
    // Authenticate and authorize
    const ctx = await requireAuthenticatedUser(req, insforge);
