...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.recordedBy === "client") {
      const { data: existingOutcome, error: existingError } = await ctx.supabase
        .from("outcomes")
        .select("id")
    ...
    const assessment = await loadAssessmentForSession(session, ctx.supabase);
    ...
    const { data: insertedOutcome, error: insertError } = await ctx.supabase
      .from("outcomes")
    ...
    const { data: trendOutcomes, error: trendError } = await ctx.supabase
      .from("outcomes")
