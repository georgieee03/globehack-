...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    switch (body.action) {
      case "aggregate":
        result = await getAggregateMetrics(ctx.supabase, clinicId, _dateRange);
        break;
      case "practitioner":
        result = await getPractitionerMetrics(ctx.supabase, clinicId);
        break;
      case "protocol":
        result = await getProtocolEffectiveness(ctx.supabase, clinicId);
        break;
      case "device":
        result = await getDeviceUtilization(ctx.supabase, clinicId);
        break;
      case "retention":
        result = await getRetentionMetrics(ctx.supabase, clinicId);
        break;
      case "roi":
