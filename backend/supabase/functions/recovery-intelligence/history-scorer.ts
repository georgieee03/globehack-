...
// ---------------------------------------------------------------------------
// scoreSession — Score a single session outcome on a 0.0–1.0 scale
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// computeConfidence — Confidence score from session count
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// scoreHistory — Query and score prior sessions for a client
// ---------------------------------------------------------------------------
...
export async function scoreHistory(
  ...
  if (error || !sessions || sessions.length === 0) {
    return {
      ...
      confidence: 0.0,
      adjustments: [
        "No prior sessions — using default protocol for this goal",
      ],
      bestConfig: null,
    };
  }
  ...
  if (sessionScores.length === 0) {
    ...
    if (confidence < 0.5) {
      adjustments.push(
        "Limited history — recommendation may improve with more sessions",
      );
    }
  }
